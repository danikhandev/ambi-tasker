  ...
  const renderCard = (worker: any, i: number) => {
    ...
    return (
      <motion.div
        ...
        className="group bg-card rounded-2xl md:rounded-[32px] border border-border/60 overflow-hidden shadow-sm hover:shadow-2xl hover:border-primary/20 transition-all duration-500 flex flex-col h-full relative"
      >
        {/* Cover Image - Hidden on mobile, shown on desktop */}
        <Link href={`/provider/${worker.id}`} className="hidden md:block aspect-[16/10] relative overflow-hidden bg-muted/30">
          <Image 
        ...
        {/* Content Box */}
        <div className="p-4 md:p-6 relative flex-1 flex flex-col justify-between">
          <div className="flex flex-row md:flex-col md:items-start gap-3 md:gap-0">
            ...
            <div className="flex-1 min-w-0 md:pt-1">
              <div className="flex items-center justify-between md:justify-start gap-1.5 mb-0.5">
                <h4 className="text-xs md:text-sm font-black text-foreground uppercase tracking-tight flex items-center gap-1 truncate">
                  {worker.name}
                  {isApproved && <ShieldCheck size={12} className="text-primary shrink-0" />}
                </h4>
                {/* Mobile: Show rating inline */}
                <div className="md:hidden flex items-center gap-0.5 bg-amber-50 border border-amber-100 px-1 py-0.5 rounded">
                  <Star size={9} className="text-yellow-500 fill-yellow-500" />
                  <span className="text-[9px] font-black text-slate-800">{worker.rating}</span>
                </div>
              </div>

              {/* Mobile location & verification */}
              <div className="flex items-center gap-2 mb-1">
                <div className="flex items-center gap-0.5 text-text-hint">
                  <MapPin size={8} />
                  <span className="text-[9px] font-bold truncate max-w-[80px]">{worker.location}</span>
                </div>
                {/* Compact approval indicator for mobile */}
                {isApproved && (
                  <span className="md:hidden text-[7px] font-black uppercase text-success tracking-widest bg-success/10 px-1 py-0.2 rounded">
                    {t("search.verified") || "Verified"}
                  </span>
                )}
              </div>

              <h3 className="text-xs md:text-sm font-black text-foreground mb-0.5 group-hover:text-primary transition-colors line-clamp-1">{worker.title}</h3>
              <p className="text-[9px] md:text-[10px] text-text-hint line-clamp-1 md:line-clamp-2 md:mb-6 italic opacity-70 group-hover:opacity-100 transition-opacity">"{worker.bio}"</p>
            </div>
          </div>

          {/* Action Row - Tighter on mobile */}
          <div className="mt-3 md:mt-auto flex items-center justify-between pt-3 md:pt-5 border-t border-border/40">
            <div className="flex flex-col">
              <span className="text-[8px] font-black uppercase text-text-hint tracking-widest">{t("search.startingAt")}</span>
              <span className={`${unbounded.className} text-[11px] md:text-sm font-black text-primary`}>Rs. {worker.hourlyRate}</span>
            </div>
            {isApproved ? (
              <Link 
                href={`/provider/${worker.id}?book=true`} 
                className="px-4 md:px-6 py-2 md:py-2.5 bg-primary text-white rounded-lg md:rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-primary-dark hover:shadow-xl hover:shadow-primary/30 transition-all active:scale-95 shadow-md shadow-primary/5"
              >
                {t("search.bookNow")}
              </Link>
            ) : (
              <button 
                ...
                className="px-4 md:px-6 py-2 md:py-2.5 bg-gray-200 text-gray-500 cursor-not-allowed rounded-lg md:rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all"
              >
                {t("search.bookNow")}
              </button>
            )}
      ...
      <section className="relative pt-28 md:pt-36 pb-12 md:pb-16 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center relative z-20">
            <motion.h1 
              ...
              className={`${unbounded.className} text-3xl md:text-5xl font-black mb-3 tracking-tighter leading-tight`}
            >
            ...
            <motion.p 
              ...
              className="text-text-secondary font-medium mb-6 text-sm md:text-base"
            >
              {t("search.subtitle")}
            </motion.p>
            ...
            >
              <div className="flex bg-white/70 backdrop-blur-2xl p-1.5 md:p-2.5 rounded-[24px] md:rounded-[40px] border border-white/50 shadow-[0_20px_40px_-12px_rgba(0,0,0,0.08)] items-center focus-within:ring-4 focus-within:ring-primary/10 focus-within:border-primary/40 transition-all duration-500">
                <div className="mx-3 md:mx-6 p-2 md:p-3 bg-primary/5 rounded-xl md:rounded-2xl text-primary">
                ...
                <input 
                  ...
                  className="flex-1 px-1 md:px-2 py-3 md:py-5 bg-transparent outline-none font-bold text-sm md:text-lg placeholder:text-text-hint/40 tracking-tight"
                />
                ...
                <AnimatePresence>
                  {showSuggestions && suggestions.length > 0 && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: 10 }}
                      className="absolute top-full left-0 right-0 mt-4 bg-white/90 backdrop-blur-3xl rounded-[24px] md:rounded-[32px] border border-white/50 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)] overflow-hidden z-50 p-2"
                    >
                ...
                <div className="flex items-center gap-2 pr-1 md:pr-2">
                  <button 
                    ...
                    className={`p-2.5 md:p-4 rounded-[12px] md:rounded-[20px] transition-all duration-300 ${isListening ? "bg-red-500 text-white animate-pulse shadow-lg shadow-red-200" : "bg-muted/50 text-text-hint hover:bg-primary/10 hover:text-primary"}`}
                  >
                    <Mic size={16} className="md:w-5 md:h-5" />
                  </button>
                  <button className="px-6 md:px-12 py-3 md:py-5 bg-gradient-to-br from-primary to-indigo-600 text-white font-black rounded-[16px] md:rounded-[28px] shadow-[0_12px_24px_-8px_rgba(var(--primary-rgb),0.3)] hover:shadow-[0_16px_32px_-8px_rgba(var(--primary-rgb),0.4)] hover:-translate-y-0.5 transition-all duration-300 uppercase text-[10px] md:text-xs tracking-widest active:scale-95 group overflow-hidden relative">
              ...
              {isListening && (
                <div className="mt-3 text-[10px] font-black text-primary animate-pulse tracking-widest uppercase">
